...
static MT_FORCEINLINE __m128i makediff16_core_sse2(const __m128i &dst, const __m128i &src, const __m128i &halfrange) {
...
void makediff16_stacked_c(Byte *pDst, ptrdiff_t nDstPitch, const Byte *pSrc, ptrdiff_t nSrcPitch, int nWidth, int nHeight)
{
    ...
    for (int y = 0; y < nHeight / 2; y++) {
        for (int x = 0; x < nWidth; x++) {
            ...
            Word result = makediff16_core_c(dst, src);
...
void makediff16_interleaved_c(Byte *pDst, ptrdiff_t nDstPitch, const Byte *pSrc, ptrdiff_t nSrcPitch, int nWidth, int nHeight)
{
    for (int y = 0; y < nHeight; y++) {
        ...
        for (int x = 0; x < nWidth / 2; x++) {
            pDstWord[x] = makediff16_core_c(pDstWord[x], pSrcWord[x]);
        }
        pDst += nDstPitch;
        pSrc += nSrcPitch;
    }
...
void makediff16_stacked_sse2(Byte *pDst, ptrdiff_t nDstPitch, const Byte *pSrc, ptrdiff_t nSrcPitch, int nWidth, int nHeight)
{
    ...
    for ( int j = 0; j < nHeight / 2; ++j ) {
        for ( int i = 0; i < wMod8; i+=8 ) {
            ...
            auto result = makediff16_core_sse2(dst, src, halfrange);
    ...
    if (nWidth > wMod8) {
        makediff16_stacked_c(pDst2 + wMod8, nDstPitch, pSrc2 + wMod8, nSrcPitch, nWidth - wMod8, nHeight);
    }
}

void makediff16_interleaved_sse2(Byte *pDst, ptrdiff_t nDstPitch, const Byte *pSrc, ptrdiff_t nSrcPitch, int nWidth, int nHeight)
{
    ...
    if (nWidth > wMod16) {
        makediff16_interleaved_c(pDst2 + wMod16, nDstPitch, pSrc2 + wMod16, nSrcPitch, nWidth - wMod16, nHeight);
    }
}
